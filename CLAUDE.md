...
│       ├── stock.py            ← GET /api/stock + /dashboard + /kline + /kd + /macd + /institutional
│       ├── market_analysis.py  ← GET /api/analyze
│       ├── taiex.py            ← GET /api/taiex
│       ├── sectors.py          ← GET /api/sectors
│       └── movers.py           ← GET /api/movers
├── data/
│   ├── tw_universe.json        ← 40 支台股
│   └── us_universe.json        ← 25 支美股
├── vercel.json
└── requirements.txt
```
...
```env
VITE_API_BASE=               # 留空 = 用 Vite proxy 到 127.0.0.1:8001
# 生產環境填入 Vercel URL，例如 https://stock-radar-api.vercel.app/api
...
- [ ] 高優先：填入 Firebase 憑證（`.env`）→ 自選股跨裝置同步
- [ ] 高優先：部署 `stock-api/` 到 Vercel → 取得生產 API URL
- [ ] 中優先：部署 `stock-frontend/` 到 GitHub Pages
- [ ] 低優先：美股個股儀表板支援（目前只支援台股 symbol）
- [ ] 低優先：自選股支援美股代號（AAPL、NVDA 等）
